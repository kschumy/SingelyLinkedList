...
func main() {
	...
	middle, _ := myLinkedList.FindMiddleValue()
	if middle != 4 {
	...
	fmt.Println("Verifying the reversed linked list by calling find_nth_from_beginning method.")

	value,_ = myLinkedList.FindNthFromBeginning(2)
	if value != 3 {
		fmt.Println("BUG: Value at index 2 should be 3 and is",  value)
	}

	value, _ = myLinkedList.FindNthFromBeginning(1)
	if value != 4 {
		fmt.Println("BUG: Value at index 1 should be 4 and is",  value)
	}

	value, _ = myLinkedList.FindNthFromBeginning(0)
	if value != 6 {
		fmt.Println("BUG: Value at index 0 should be 6 and is",  value)
	}


	// nth from the end
	fmt.Println("Verifying find_nth_from_end method.")
	value, _ = myLinkedList.FindNthFromEnd(0)
	if value != 3 {
		fmt.Println("BUG: Value at index 0 from the end, should be 3 and is",  value)
	}

	value, _ = myLinkedList.FindNthFromEnd(1)
	if value != 4 {
		fmt.Println("BUG: Value at index 1 from the end, should be 4 and is",  value)
	}

	value, _ = myLinkedList.FindNthFromEnd(2)
	if value != 6 {
		fmt.Println("BUG: Value at index 2 from the end, should be 6 and is",  value)
	}

	// check for cycle
	fmt.Println("Checking the linked list for cycle.")
	if myLinkedList.HasCycle() {
		fmt.Println("BUG: Should not have a cycle.")
	}
